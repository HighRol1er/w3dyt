...
export class BinanceService {
  ...
  async getAllMarketPrices(): Promise<BinanceTicker[]> {
    try {
      const response = await axios.get<BinanceTicker[]>(`${this.BINANCE_API_URL}/ticker/24hr`);

      // USDT 마켓만 필터링
      const usdtMarkets = response.data.filter(ticker => ticker.symbol.endsWith('USDT'));
  ...
  async getSimplePrices(): Promise<BinancePrice[]> {
    try {
      const response = await axios.get<BinancePrice[]>(`${this.BINANCE_API_URL}/ticker/price`);

      // USDT 마켓만 필터링
      const usdtMarkets = response.data.filter(price => price.symbol.endsWith('USDT'));
